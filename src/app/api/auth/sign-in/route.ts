...
export async function POST(request: NextRequest) {
  if (!API_BASE) {
    return NextResponse.json(
      { message: "API base URL not configured (NEXT_PUBLIC_API_BASE_URL)." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    const signInResp = await fetch(`${API_BASE}sign-in-attempt`, {
      method: "POST",
      headers: {
    ...
    const contentType = signInResp.headers.get("content-type") || "";
    const data = contentType.includes("application/json")
      ? await signInResp.json()
      : { message: await signInResp.text() };

    return NextResponse.json(data, { status: signInResp.status });
  } catch (err) {
    console.error("[api/auth/sign-in] proxy error", err);
    return NextResponse.json(
      { message: "Sign-in proxy failed.", error: String(err) },
      { status: 502 }
    );
  }
}
